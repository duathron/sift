...
- **`--format html` shift-handover report:** `sift triage … --format html` produces a self-contained dark-themed HTML document (embedded CSS, no external dependencies) with cluster cards, priority badges, IOC tables with type annotations, alert tables, and ATT&CK technique listings. Implemented in `sift/output/html.py`. Redacted fields are never re-surfaced. (S3)
- **`--format md` shift-handover report:** `sift triage … --format md` produces a Markdown document suitable for Jira/Confluence, ticket attachments, or shift-handover repos. Contains metadata table, executive summary (if present), and per-cluster sections with IOC and alert tables. Implemented in `sift/output/md.py`. Redacted fields are never re-surfaced. (S3)
